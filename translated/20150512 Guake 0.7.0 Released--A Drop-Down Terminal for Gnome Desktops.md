...
您可以从下面的链接中获得一份使用最管饭的终端模拟器的列表。

- [20 Useful Terminals for Linux][1]

前几日上网时，我偶遇了名为‘guake’的终端，它是gnome下的终端。尽管这并不是我第一次听到Guake。实际上，我在大约一年前便知道了这个应用程序，但不知怎么搞的，我那时没有写写Guake，后来我渐渐忘掉了Guake，直到我再一次听到Guake。所以，最终，这篇文章诞生了。我将给你讲讲Guake的功能，在Debian、Ubuntu、Fedora上的安装过程以及一些测试。

#### 什么是Guake？ ####

Guake是应用于Gnome环境的下拉式终端。主要由Python编写，使用了一些C，它以GPL2+证书发布，适用于Linux和相似的系统。Guake的灵感来源于电脑游戏Quake（雷神之锤）的终端，Quake的终端能通过按下特定按键（默认为F12）从屏幕上滑下来，并在按下同样的键后滑上去。

值得注意的是，Guake并不是第一个这样的应用。Yakuake（Yet Another Kuake）是一个运行于KDE的终端模拟器，Tilda是一个GTK+写成的终端模拟器。它们的灵感都来自于雷神之锤那上下滑动的终端。
...
- 平滑的集成于GUI
- 在按下预定义的键后出现/消失
- 支持热键、tabs、透明化背景，这使得它成为适合所有Gnome用户的精彩应用
- 极端地可配置
- 包括许多颜色的调色板
- 设定透明度的快捷键
...
如果您的壁纸或活动窗口的颜色和Guake的颜色不匹配。您可以改变您的壁纸或者减少透明度或者改变Guake的颜色。
...
我认为对任何同时使用GUI和终端的Linux用户来说，它都是必须的工具。同样的，我会向任何想要在系统中结合使用GUI和终端的人推荐它，因为它是平滑且没有任何障碍的。
